...
function getStatusDisplay(status: string, rentalEnd: string) {
  const now = new Date()
  ...
  const map: Record<string, any> = {
    pending: { label: 'Menunggu', bg: 'bg-yellow-50', border: 'border-yellow-200', text: 'text-yellow-700', dot: 'bg-yellow-500' },
          ...
          <div className="space-y-3">
            {initialOrders.length === 0 ? (
              ...
              initialOrders.map((order, idx) => {
                ...
                const status = getStatusDisplay(order.status, order.rental_end)
