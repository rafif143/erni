...
export default function EditDressModal({ isOpen, onClose, onSuccess, dress }: EditDressModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setImagePreview(dress.image_url || '')
      setImageFile(null)
      setError('')
    }
  }, [isOpen, dress])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (dress.image_url) {
          await deleteDressImage(dress.image_url)
        }
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-sm font-semibold text-gray-700">Tersedia untuk disewa</span>
            </label>
          </div>
