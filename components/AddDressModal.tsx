...
export default function AddDressModal({ isOpen, onClose, onSuccess }: AddDressModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('dresses').insert({
      ...
      setFormData({ name: '', description: '', price: '', category: '', stock: '', size: [] })
          ...
          <div className="sticky top-0 bg-white border-b border-pink-100 px-8 py-6 rounded-t-2xl z-10">
            <div className="flex justify-between items-center">
              <h2 className="font-playfair text-2xl font-bold text-gray-900">
                Tambah <em className="italic text-[#e8628a]">Kostum</em>
              </h2>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-xs text-gray-400"><p>Format: JPG, PNG, WEBP, GIF</p><p>Maksimal: 5MB</p></div>
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                className="modal-input w-full bg-[#f8f5f7] border border-pink-100 rounded-lg px-4 py-3.5 text-[0.85rem] text-gray-900 transition-colors duration-200" placeholder="Contoh: Kostum Superhero" required />
            </div>
            ...
            <div>
              ...
              <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3}
                className="modal-input w-full bg-[#f8f5f7] border border-pink-100 rounded-lg px-4 py-3.5 text-[0.85rem] text-gray-900 transition-colors duration-200 resize-none" placeholder="Deskripsi detail kostum..." />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                  className="modal-input w-full bg-[#f8f5f7] border border-pink-100 rounded-lg px-4 py-3.5 text-[0.85rem] text-gray-900 transition-colors duration-200" placeholder="Contoh: Superhero" required />
              </div>
              <div>
                ...
                <input type="number" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })}
                  className="modal-input w-full bg-[#f8f5f7] border border-pink-100 rounded-lg px-4 py-3.5 text-[0.85rem] text-gray-900 transition-colors duration-200" placeholder="100000" required min="0" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="number" value={formData.stock} onChange={(e) => setFormData({ ...formData, stock: e.target.value })}
                className="modal-input w-full bg-[#f8f5f7] border border-pink-100 rounded-lg px-4 py-3.5 text-[0.85rem] text-gray-900 transition-colors duration-200" placeholder="10" required min="0" />
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {sizes.map((size) => (
                  <button key={size} type="button" onClick={() => handleSizeToggle(size)}
                    className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${formData.size.includes(size)
                      ? 'bg-[#e8628a] text-white shadow-sm'
                      : 'bg-[#f8f5f7] border border-pink-100 text-gray-500 hover:bg-pink-50 hover:text-gray-700'
                      }`}>{size}</button>
                ))}
              </div>
              {formData.size.length === 0 && <p className="text-xs text-red-500 mt-1.5">Pilih minimal 1 ukuran</p>}
            </div>
